...
class Client implements IClient, Inspectable
{
    ...
    protected function _handleWriteBuffer(link\peer\ISession $session)
    {
        ...
        } elseif ($session->getStore('writeListen')) {
            return link\peer\IIoState::WRITE_LISTEN;
        } else {
    ...
    public function glitchInspect(Entity $entity, Inspector $inspector): void
    {
        $entity
            ->setProperties([
                '*isRunning' => $inspector($this->isRunning()),
                '*chunkSize' => $inspector($this->_readChunkSize.' / '.$this->_writeChunkSize),
                '*sessions' => $inspector($this->_sessionCount),
                '*dispatcher' => $inspector($this->events)
            ]);
    }
}
