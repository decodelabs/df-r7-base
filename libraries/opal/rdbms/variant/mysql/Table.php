...
// Alter
    protected function _alter(opal\rdbms\schema\ISchema $schema) {
        ...
        $sql = array();
        $mainSql = 'ALTER TABLE '.$this->_adapter->quoteIdentifier($this->_name);
...
// Triggers
    protected function _generateTriggerDefinition(opal\rdbms\schema\ITrigger $trigger) {
        ...
        return parent::_generateTriggerDefinition($trigger);
    }
    ...
    protected function _introspectSchema() {
        ...
        if($res->isEmpty()) {
            throw new opal\rdbms\TableNotFoundException(
                'Table '.$this->_name.' could not be found', 1051, $sql
            );
        }
        ...
        // Columns
        $sql = 'SHOW FULL COLUMNS FROM '.$this->_adapter->quoteIdentifier($this->_name);
        ...
        // Indexes
        $sql = 'SHOW INDEXES FROM '.$this->_adapter->quoteIdentifier($this->_name);
        $res = $this->_adapter->prepare($sql)->executeRead();
