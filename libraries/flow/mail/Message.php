...
// Recipients
    ...
    public function addRecipient($to, $name=null) {
        ...
        } else if($to instanceof IAddressList) {
        ...
        if(is_array($to) || $to instanceof \ArrayAccess) {
            if(isset($to['id'])) {
                return $this->addToUser($to);
            } else if(isset($to['email'])) {
                if(!$name && isset($to['fullName'])) {
                    $name = $to['fullName'];
                } else if(!$name && isset($to['name'])) {
                    $name = $to['name'];
                }
...
// To users
    ...
    public function hasToUser(...$ids): bool {
        foreach($ids as $id) {
            if($id instanceof user\IClientDataObject) {
                $id = $id->getId();
            } else if((is_array($id) || $id instanceof \ArrayAccess) && isset($id['id'])) {
                $id = $id['id'];
            }
...
// Attachments
    ...
    public function attachString($string, string $contentType=null, string $contentId=null): IAttachment {
        return $this->attach(new core\fs\MemoryFile($string, $contentType), $contentId);
    }
